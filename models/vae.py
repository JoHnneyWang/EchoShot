...
class VideoVAE:
    ...
    def encode(self, videos, inner_t=None):
        """
        ...
        with amp.autocast(dtype=self.dtype):
            if inner_t is not None:
                z_list = []
                for u, t in zip(videos, inner_t):
                    fr = [(t_-1)*4+1 for t_ in t]
                    start_fr = [sum(fr[:i]) for i in range(len(fr))]
                    end_fr = [sum(fr[:i]) for i in range(1, len(fr)+1)]
                    z_fr_list = []
                    for s, e in zip(start_fr, end_fr):
                        z_fr_list.append(self.model.encode(u[:,s:e].unsqueeze(0), self.scale).float().squeeze(0))
                    z_list.append(torch.cat(z_fr_list, dim=1))
                return z_list
                ...
                return [self.model.encode(
                    u, self.scale)
                .float() for u in videos]
    
    def decode(self, zs, inner_t=None): 
        with amp.autocast(dtype=self.dtype):

            if inner_t is not None:
                z_list = []
                for u, t in zip(zs, inner_t):
                    start_t = [sum(t[:i]) for i in range(len(t))]
                    end_t = [sum(t[:i]) for i in range(1, len(t)+1)]
                    z_fr_list = []
                    for s, e in zip(start_t, end_t):
                        z_fr_list.append(self.model.decode(u[:,s:e].unsqueeze(0), self.scale).float().squeeze(0))
                    z_list.append(torch.cat(z_fr_list, dim=1))
                return z_list
